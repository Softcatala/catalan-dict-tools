  ...
  LINE: while ( my $modelline = <$modelfh> ) {
            ...
            && $modelline !~ /IGNOREHUNSPELL/ )
        {
            ...
            if ($general) {
                ...
                next LINE if ( $postag =~ /^V.N.*/
                    && $forma =~ /é(ixer|nyer|ncer)$/
                    && $forma !~ /^(cr|acr|decr|n|p|recr|ren|sobrecr|sobren)éixer$/ );
            ...
            if ( $postag =~ /^V.N.*$/ ) {
                ...
                $afixos = "_Y"
                  ; #participi FS **** Falta afegir l'apostrofació l' (_V) en la forma evitant les excepcions.
            }
            elsif ( $postag =~ /^V.P..P..$/ ) {
