...
my @case;
$case[0] = "MS"; $case[1] = "FS"; $case[2] = "MP"; $case[3] = "FP";
$case[4] = "MS"; $case[5] = "FS"; $case[6] = "MP"; $case[7] = "FP";
...
while ( my $line = <$fh> ) {
    ...
    if ( $line =~ /^([^ ]+) (A.+|N.+) ([^ ]+)$/ ) {
	    ...
	    $numAccepcio="";
	    if ($prevLemma !~ /^MP3|A[345]|goma-2$/) {  #Excepcions: el número forma part de la paraula
		if ($prevLemma =~ /([0-9])$/) {
		    $numAccepcio = $1;
		}
	    }
        ...
        if ( $postag =~ /[MC][SN]/ ) {
            ...
            elsif ( ($forma[4] =~ /^$/) && ( $word !~ /^$forma[0]$/ )) { $forma[4] = $word; }
        ...
        if ( $postag =~ /[FC][SN]/ ) {
            ...
            elsif ( ($forma[5] =~ /^$/) && ( $word !~ /^$forma[1]$/ ) ) { $forma[5] = $word; }
        ...
        if ( $postag =~ /[MC][PN]/ ) {
            ...
            elsif ( ($forma[6] =~ /^$/) && ( $word !~ /^$forma[2]$/ ) ) { $forma[6] = $word; }
        ...
        if ( $postag =~ /[FC][PN]/ ) {
            ...
            elsif ( ($forma[7] =~ /^$/) && ( $word !~ /^$forma[3]$/ ) ) { $forma[7] = $word; }
...
=cut
    ...
    if ($modeltag =~ /(NC|AQ0|AQA|AO0)/) # És nom, adjectiu o adjectiu ordinal
    {
	my $originTag=$1;

	#nom masculí
	if ($modeltag !~ /(NC|AQ0|AQA|AO0)[FC]/ 
	...
	elsif ($modeltag =~ /(NC|AQ0|AQA|AO0)C/ 
	       || ($modeltag =~ /(NC|AQ0|AQA|AO0)F/ && $modeltag =~ /(NC|AQ0|AQA|AO0)M/) )
	{
	    if ($forma[4] =~ /^$/) 
	    {
		...
		{
		    print $ofh "$forma[0]$numAccepcio $forma[1]"; #agarè agarena
		}
	    } 
	    else 
	    { 
		print $ofh "$forma[0]$numAccepcio $forma[1] [masc. $forma[4]]";  # bon
	    }
		...
		# o excepcional: coipú/coipús
		# invariables no
		elsif (Flexio::mascplural_partintdeduesformes($forma[0], $forma[1]) !~ /^$forma[2]$/)
		{
		    if ($forma[6] =~ /ès$/) 
		    {
			print $ofh " [pl. $forma[6]]";
	...
	#nom masculí singular
	       ...
	       && $modeltag =~ /(NC|AQ0|AQA|AO0)MS/ && $modeltag !~ /(NC|AQ0|AQA|AO0)M[NP]/ )
	{
	...
	#nom masculí plural
	       ...
	       && $modeltag =~ /(NC|AQ0|AQA|AO0)MP/ && $modeltag !~ /(NC|AQ0|AQA|AO0)M[NS]/ )
	{
	...
	#nom femení singular
	       ...
	       && $modeltag =~ /(NC|AQ0|AQA|AO0)FS/ && $modeltag !~ /(NC|AQ0|AQA|AO0)F[NP]/ )
	{
	...
	#nom femení plural
	       ...
	       && $modeltag =~ /(NC|AQ0|AQA|AO0)FP/ && $modeltag !~ /(NC|AQ0|AQA|AO0)F[NS]/ )
	{
