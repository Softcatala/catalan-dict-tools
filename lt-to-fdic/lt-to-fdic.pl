...
my $arxiucategoria = $ARGV[0];#
adjectius, noms
my $dir_entrada = $ARGV[1];

my $f1 = $dir_entrada.
"/ordenats-".$arxiucategoria.
".txt";

#
my $out = "models_flexio_nominal.txt";
my $out2 = $dir_entrada.
"/mots_no_processats.txt";
my $out3 = $dir_entrada.
"/".$arxiucategoria.
"-fdic.txt";
  ...
  if (
    ...
    if (
      $modeltag!~/(NC|AQ0|AQA|AO0)[FC]/ && (
        $modeltag = ~/(NC|AQ0|AQA|AO0)MN/ || ($modeltag = ~/(NC|AQ0|AQA|AO0)MS/ && $modeltag = ~/(NC|AQ0|AQA|AO0)MP/)
      )
    ) {
      ...
      elsif($forma[0] = ~/è[sn]?$/)# entre bebè i bebé tria només l 'accentuació general {
        print $ofh "$forma[0]$numAccepcio";
      } else {
      ...
      #
      dues formes de plural(desig / desitjos, mig / mitjos, enuigs / enutjos)# però no cal "textos/texts, balancejos/balanceigs, assaigs/assajos, puigs/pujos"#
      excepte invariables
      if (
        $forma[0] !~/^$forma[2]$/ && !(
        ...
        #
        plural amb dues esses: afàs afassos# o excepcional: coipú / coipús# més excepcions: amfiox amfioxs# invariables no
    ...
    nom femení
      ...
      $modeltag!~/(NC|AQ0|AQA|AO0)[MC]/ && (
        $modeltag = ~/(NC|AQ0|AQA|AO0)FN/ || ($modeltag = ~/(NC|AQ0|AQA|AO0)FS/ && $modeltag = ~/(NC|AQ0|AQA|AO0)FP/)
      )
    ) {
      ...
      elsif($forma[1] = ~/è[sn]?$/)# entre bebè i bebé tria només l 'accentuació general {
        print $ofh "$forma[1]$numAccepcio";
      } else {
      ...
      #
      plural amb dues esses: afàs afassos# o excepcional: coipú / coipús# invariables no
      elsif(($forma[1] !~/^$forma[3]$/) && (Flexio::plural($forma[1], "F", "F") !~/^$forma[3]$/)) {
        print $ofh " [pl. $forma[3]]";
      }
    ...
    nom masculí i femení
      ...
      $modeltag = ~/(NC|AQ0|AQA|AO0)C/ || ($modeltag = ~/(NC|AQ0|AQA|AO0)F/ && $modeltag = ~/(NC|AQ0|AQA|AO0)M/)
    ) {
      if ($forma[4] = ~/^$/) {
        ...
        if ($forma[1] = ~/^$/ || $forma[0] = ~/^$forma[1]$/) {
          ...
          print $ofh
            "$forma[0]$numAccepcio $forma[1]";#
          agarè agarena
        }
      } else {
        print $ofh
          "$forma[0]$numAccepcio $forma[1] [masc. $forma[4]]";#
        bon
      }
    ...
    elsif($modeltag!~/(NC|AQ0|AQA|AO0)[FC]/ && $modeltag = ~/(NC|AQ0|AQA|AO0)MS/ && $modeltag!~/(NC|AQ0|AQA|AO0)M[NP]/) {
    ...
    #
    nom masculí plural
    elsif($modeltag!~/(NC|AQ0|AQA|AO0)[FC]/ && $modeltag = ~/(NC|AQ0|AQA|AO0)MP/ && $modeltag!~/(NC|AQ0|AQA|AO0)M[NS]/) {

      if ($forma[6] = ~/^$/) {
        print $ofh "$forma[2]$numAccepcio";
      }
      elsif($forma[2] = ~/è[sn]?$/)# entre bebè i bebé tria només l 'accentuació general {
        print $ofh "$forma[2]$numAccepcio ???";
      } else {
    ...
    #
    nom femení singular
    elsif($modeltag!~/(NC|AQ0|AQA|AO0)[MC]/ && $modeltag = ~/(NC|AQ0|AQA|AO0)FS/ && $modeltag!~/(NC|AQ0|AQA|AO0)F[NP]/) {
    ...
    #
    nom femení plural
    elsif($modeltag!~/(NC|AQ0|AQA|AO0)[MC]/ && $modeltag = ~/(NC|AQ0|AQA|AO0)FP/ && $modeltag!~/(NC|AQ0|AQA|AO0)F[NS]/) {

      if ($forma[7] = ~/^$/) {
        print $ofh "$forma[3]$numAccepcio";
      }
      elsif($forma[3] = ~/è[sn]?$/)# entre bebè i bebé tria només l 'accentuació general {
        print $ofh "$forma[3]$numAccepcio ???";
      } else {
